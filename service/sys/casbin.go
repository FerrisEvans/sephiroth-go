...
func (casbinService *CasbinService) UpdateCasbin(AuthorityID uint, casbinInfos []req.CasbinInfo) error {
...
func (casbinService *CasbinService) GetPolicyPathByAuthorityId(AuthorityID uint) (pathMaps []req.CasbinInfo) {
	e := casbinService.Casbin()
	authorityId := strconv.Itoa(int(AuthorityID))
	list, _ := e.GetFilteredPolicy(0, authorityId)
	for _, v := range list {
		pathMaps = append(pathMaps, req.CasbinInfo{
			Path:   v[1],
			Method: v[2],
		})
	}
...
func (casbinService *CasbinService) Casbin() *casbin.SyncedCachedEnforcer {
	once.Do(func() {
		a, err := gormadapter.NewAdapterByDB(core.Db)
		if err != nil {
